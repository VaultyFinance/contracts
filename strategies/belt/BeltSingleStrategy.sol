...
import "./interface/IStrategyToken.sol";
...
contract BeltSingleStrategy is BaseUpgradeableStrategy {
    ...
    address[] public pancake_BELT2TOKEN; // route to exchange BELT on vault token (BTCB, ETH, etc)
    ...
    function initializeStrategy(
        ...
        //address vtoken = IStrategyToken(_depositHelp).token();
        //require(_vaultToken == vtoken, "Vault token mismatch");
        _setPoolId(_poolID);
    ...
    function claimAndLiquidateReward() internal {
        ...
        _liquidateReward();
        uint256 tokenBalance = IBEP20(vaultToken).balanceOf(address(this));
        if (tokenBalance > 0) {
            IBEP20(vaultToken).safeApprove(depositor(), 0);
            IBEP20(vaultToken).safeApprove(depositor(), tokenBalance);            
        }
    }
    ...
    function doHardWork() external onlyNotPausedInvesting restricted {
        ...
        if (bal != 0) {
            claimAndLiquidateReward();
            uint256 tokenBalance = IBEP20(vaultToken).balanceOf(address(this));
            IStrategyToken(depositor()).deposit(poolId(), tokenBalance);
        }
        investAllUnderlying();
    }
